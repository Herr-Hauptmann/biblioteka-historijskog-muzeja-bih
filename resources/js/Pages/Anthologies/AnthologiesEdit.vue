...
const submit = () => {
    form.post(route("anthologies.update", props.anthology.id))
}
...
<template>
    <Head title="Izmjena broja — Zbornik radova Historijskog muzeja BiH" />
    <Content>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Izmjena broja zbornika radova Historijskog muzeja BiH</h2>
        </template>
                        ...
                        <div class="grid gap-6 mb-6 md:grid-cols-2">
                            ...
                            <div>
                                <div v-if="showUpload">
                                    ...
                                    <input name="anthology" @input="form.anthology = $event.target.files[0]"
                                        class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50"
                                        aria-describedby="anthology_help" id="anthology" type="file">
                                ...
                                <div v-else>
                                    <a :href="route('anthologies.show', anthology.id)" target="_blank"
                                        class="flex flex-col items-center">
                                        <img class="h-12 w-12" :src="pdf_icon" alt="Postojeći PDF broja zbornika">
                                        <p class="mt-2">{{ anthology.file_name }}</p>
                                    </a>
                                    <button type="button"
